...
--load log
logs = load '/kvlogs/$CHN/$YEAR/$MONTH/*$YEAR$MONTH$DAY*.log'
...
--userItemCountLarge = filter userItemCount by count >=10;
--userItemCountLog = foreach userItemCountLarge generate uuid, id, LOG10(count) as count;
...
both = union discounted, userItemCountLarge;
big = filter both by count>5;
...
--calc relation
logcount = foreach sumed generate uuid, id, LOG10(count)
usergrp = group logcount by uuid;
describe usergrp;
total = foreach usergrp generate group as uuid, SUM(logcount.count) as total;
precross = foreach usergrp generate flatten(wonderyao.CrossSelf(logcount));
